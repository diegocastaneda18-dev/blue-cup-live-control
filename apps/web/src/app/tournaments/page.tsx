    ...
    <span
      className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${
        on
          ...
          : "border-maria-pearl/10 bg-maria-pearl/5 text-maria-sand/70"
      }`}
    >
      {label}: {on ? "Yes" : "No"}
    </span>
  ...
  return (
    <main className="mx-auto max-w-4xl p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="font-display text-3xl font-semibold tracking-tight text-maria-pearl">
            Blue Cup Live Control
          </h1>
          <p className="mt-2 max-w-2xl text-maria-sand/75">
            Tournaments from the API at <span className="text-maria-pearl">{API_TOURNAMENTS}</span>.
          </p>
          {demoFallback ? (
            <p className="mt-3 max-w-2xl rounded-xl border border-maria-sunset/30 bg-maria-sunset/10 px-4 py-3 text-sm leading-relaxed text-maria-sunset-light">
